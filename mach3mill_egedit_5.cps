...
description = "Mach3Mill_EGEDIT_v4";
vendor = "Artsoft";
...
properties = {
    ...
    optionalStop: false, // optional stop  egemen edit
...
function onOpen() {
    ...
    //EGEMEN write a file containing tools
/*
    var outputPath = getOutputPath();
    var programFilename = FileSystem.getFilename(outputPath);
    var outputP = FileSystem.getFolderPath(outputPath);
    op = outputP + "\\" + programFilename.substr(0, programFilename.length - 4) + ".txt";
    ...
    if (tools.getNumberOfTools() > 0) {
        for (var i = 0; i < tools.getNumberOfTools(); ++i) {
            ...
            var comment = toolFormat.format(tool.number) + "  " + xyzFormat.format(tool.diameter) + " milimeter " + getToolTypeName(tool.type);
...
*/
    ...
    //EGEMEN EDIT
    //writeBlock(mFormat.format(231));  //START DUST COLLECTION
    writeBlock(mFormat.format(261));  //MACHINE RUNNING FLAG
    //EGEMEN EDIT
...
function onSection() {
    ...
    if (insertToolCall) {
        ...
        if (properties.useM6) {
            ...
            //Alert telegram for toolchange
            writeBlock(mFormat.format(251));
            writeBlock("G04 P1");
            writeBlock(mFormat.format(250));
    ...
    if (insertToolCall || retracted) {
        ...
        if (!machineConfiguration.isHeadConfiguration()) {
            ...
            //EGEMEN EDIT DISABLE G43 APPLY TOOL OFFSET
...
var mapCommand = {
    ...
    /*
    //M230-M240-M250-M260
    COMMAND_EGE_DUSTCOLLECTION_ON: 231,
    COMMAND_EGE_DUSTCOLLECTION_OFF: 230,
    COMMAND_EGE_CYCLE_START: 261,
    COMMAND_EGE_CYCLE_END: 260,
    COMMAND_EGE_TOOLCHANGE_ON: 251,
    COMMAND_EGE_TOOLCHANGE_OFF: 250
...
function onClose() {
    ...
    writeBlock(mFormat.format(230));  //Turn off Dust Collection
    writeBlock(mFormat.format(260));  //Turn off output 6 Telegram message
    ...
    writeBlock("G59 P253 G00 X0 Y0 Z0");
    writeBlock(gFormat.format(53 + currentWorkOffset));
    ...
    //EGEMEN EDIT
    writeBlock(mFormat.format(230));
    writeBlock(mFormat.format(260));
